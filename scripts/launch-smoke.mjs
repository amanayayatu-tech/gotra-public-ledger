...
async function runBrowserSmoke(args, ledger, contentIndex) {
  ...
  try {
    ...
    const routes = [
      ...
      { label: "monthly_reports", hash: "#/monthly-reports", viewport: desktop, requiredText: ["月度透明报告", "错误案例", "数据缺口", "改进事项"] },
    ...
    for (const route of routes) {
      ...
      if (route.label === "symbol_profile") {
        const shot = path.join(args.outDir, "desktop-symbol-profile-1440x900.png");
        await captureScreenshot(client, shot);
        report.screenshots.push(shot);
      }
      if (route.label === "monthly_reports") {
        const shot = path.join(args.outDir, "desktop-monthly-reports-1440x900.png");
        await captureScreenshot(client, shot);
        report.screenshots.push(shot);
      }
